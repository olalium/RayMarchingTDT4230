...
namespace Gloom
{
    ...
    public:
        Camera(glm::vec3 position         = glm::vec3(0.0f, 0.0f, 0.0f),
               ...
               GLfloat   mouseSensitivity = 0.02f)
        {
            cPosition         = position;
        ...
        void updateCamera(GLfloat deltaTime)
        {
            ...
            if (keysInUse[GLFW_KEY_R])  // pitch up
                fPitch -= 0.1;

            if (keysInUse[GLFW_KEY_F])  // pitch down
                fPitch += 0.1;

            if (keysInUse[GLFW_KEY_C])  // jaw right
                fYaw += 0.1;

            if (keysInUse[GLFW_KEY_X])  // jaw left
                fYaw -= 0.1;
